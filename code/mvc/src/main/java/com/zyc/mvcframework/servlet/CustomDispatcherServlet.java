package com.zyc.mvcframework.servlet;

import com.zyc.mvcframework.annotations.*;
import com.zyc.mvcframework.pojo.Handler;
...
public class CustomDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            if(!aClass.isAnnotationPresent(CustomController.class)) {continue;}

            // 获取Controller类上的@Security标注的权限属性
            Set<String> controllerSecurityValues = new HashSet<>();
            if (aClass.isAnnotationPresent(Security.class)){
                controllerSecurityValues.addAll(
                        Arrays.stream(aClass.getAnnotation(Security.class).value())
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //  方法没有标识CustomRequestMapping，就不处理
                if(!method.isAnnotationPresent(CustomRequestMapping.class)) {continue;}
                ...
                // 获取方法上的@Security注解标注的权限属性
                Set<String> methodSecurityValues = new HashSet<>();
                if (method.isAnnotationPresent(Security.class)){
                    Security methodSecurity = method.getAnnotation(Security.class);
                    methodSecurityValues.addAll(Arrays.stream(methodSecurity.value()).collect(Collectors.toSet()));
                }
                // 得到最终handler method的权限
                Set<String> handlerSecuritys = new HashSet<>();
                handlerSecuritys.addAll(controllerSecurityValues);
                handlerSecuritys.addAll(methodSecurityValues);

                handler.setSecuritys(handlerSecuritys);
    ...
    private void doAutoWired() {
        ...
        // 遍历ioc中所有对象，查看对象中的字段，是否有@CustomAutowired，如果有需要维护依赖注入关系
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            // 获取bean对象中的字段信息
            Field[] declaredFields = entry.getValue().getClass().getDeclaredFields();
            // 遍历判断处理
            for (int i = 0; i < declaredFields.length; i++) {
                Field declaredField = declaredFields[i];   //  @CustomAutowired  private IDemoService demoService;
    ...
    private void doInstance()  {
        ...
        try{
            ...
            for (int i = 0; i < classNames.size(); i++) {
                String className =  classNames.get(i);  // DemoController
...
//        method.invoke() //
        ...
        // 进行权限验证
        boolean hasAuthority = authentication(req, handler);
        if (!hasAuthority){
            resp.getWriter().write(String.format("当前用户无权限访问[%s]!", handler.getMethod().toString()));
            // 设置一下，否则响应后浏览器显示可能会有乱码
    ...
    // 验证是否有权限访问
    private boolean authentication(HttpServletRequest req, Handler handler) {
        Set<String> handlerSecuritys = handler.getSecuritys();
        return handlerSecuritys.contains(req.getParameter("username"));
    }
